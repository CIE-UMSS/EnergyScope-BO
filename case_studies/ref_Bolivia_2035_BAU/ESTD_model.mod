...
param share_freight_train_max >= 0, <= 1; # %_rail,max [-]: max limit for penetration of train in freight transportation
param share_freight_boat_min  >= 0, <= 1; # %_boat,min [-]: min limit for penetration of boat in freight transportation
param share_freight_boat_max  >= 0, <= 1; # %_boat,max [-]: max limit for penetration of boat in freight transportation
param share_freight_road_min  >= 0, <= 1; # %_road,min [-]: min limit for penetration of truck in freight transportation
param share_freight_road_max  >= 0, <= 1; # %_road,max [-]: max limit for penetration of truck in freight transportation
...
subject to end_uses_t {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [l, h, td] = (if l == "ELECTRICITY" 
		then
			...
			(end_uses_input["COOKING"] / total_time)
			...
			(end_uses_input["MOBILITY_FREIGHT"]   * mob_freight_time_series [h, td] / t_op [h, td] ) *  (1 - Share_freight_boat - Share_freight_road - Share_freight_train)
...
subject to Freight_shares :
	Share_freight_train + Share_freight_road + Share_freight_boat = 0.862134642; # Should not be required (redundant)... But kept for security
